...
export default function EnviarAccesoUsuario({ email, nombre }: Props) {
  ...
  if (estado === "error") {
    return (
      <button onClick={() => setEstado("idle")} style={{ ...btnStyle, borderColor: "#e05c5c", color: "#e05c5c" }}>
        Error — reintentar
      </button>
    );
  }
  ...
  if (estado === "confirmando") {
    return (
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <button onClick={enviar} style={{ ...btnStyle, borderColor: "var(--oro)", color: "var(--oro)" }}>
          Confirmar
        </button>
        <button onClick={() => setEstado("idle")} style={{ ...btnStyle }}>
          Cancelar
        </button>
      </div>
    );
