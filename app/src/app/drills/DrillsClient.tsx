...
export default function DrillsClient({ drills, userAccessLevel, userViews, freeQuota }: Props) {
  ...
  const chapters = useMemo(() => [...new Set(drills.map(d => d.chapter))].sort((a, b) => a - b), [drills]);
  ...
  }, [drills, search, filterChapter, filterCategory, filterLevel]);
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 32 }}>
        ...
        <select value={filterChapter} onChange={e => setFilterChapter(e.target.value)} style={{ ...selectStyle, minWidth: 160 }}>
          ...
          {chapters.map(ch => (
            <option key={ch} value={ch}>Cap. {ch} — {drills.find(d => d.chapter === ch)?.chapter_title_es?.split(" ").slice(0, 3).join(" ")}</option>
          ))}
        </select>
        ...
        {(search || filterChapter || filterCategory || filterLevel) && (
          <button
      ...
      <p style={{ fontSize: 13, color: "var(--texto-suave)", marginBottom: 32 }}>
        ...
        {(search || filterChapter || filterCategory || filterLevel) && ` de ${drills.length}`}
      </p>
      ...
      {grouped.map(([chapter, { title, drills: capDrills }]) => (
        <div key={chapter} style={{ marginBottom: 48 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20, paddingBottom: 12, borderBottom: "1px solid var(--borde)" }}>
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--oro)", minWidth: 56 }}>Cap. {chapter}</span>
